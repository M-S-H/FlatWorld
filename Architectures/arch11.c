...
/*
	When the agent cannot see a green object, it will go towards the
	brightest object it can find. To do this, a giant WTA network has
	inputs from both green and non-green objects, with the weights for
	the non-green objects being much less than the green ones.
*/
...
{ /* Adhoc function to test agents, to be replaced with NN controller. tpc */
	...
	{		
		...
		if( nlifetimes >= maxnlifetimes )
		{
			...
			// Write out data
			FILE *fp;
			fp = fopen("./Results/Arch11 Lifetimes.csv", "w");
			for(i=0; i<maxnlifetimes; i++)
				fprintf(fp, "%d, %d\n", i, lifetimes[i]);
			fclose(fp);
